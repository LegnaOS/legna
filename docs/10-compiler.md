...
| 死代码消除 | 立即数路径回退 `_out_pos` 擦除无用 `mov` 指令 |
| 简单变量优化 | 右操作数为变量时，直接 `ldr x1` 替代 push/pop 往返 |
| 条件比较优化 | `if x > 5` 直接 `cmp x0, #5`，省去 push/pop 中间步骤 |
| for 循环优化 | 循环边界比较直接 `ldr`+`cmp`，每次迭代省 2 条指令 |
| 单参数调用优化 | 单参数函数调用跳过无意义的 push+pop 往返 |
| 乘除模优化 | `*`/`/`/`%` 运算同样消除冗余 push/pop |
| 零运行时依赖 | 不链接 libc，直接 syscall |

核心技术：**`_out_pos` 回退**——编译器顺序写入输出缓冲区，通过回退写指针擦除已发射的冗余指令，实现零开销的窥孔优化。
...
Legna 生成代码在递归场景下比 C -O0 快 27%，编译速度与 clang 持平。
